...
def _decomp_tumor_features(
    ...
    X_decomp = pca.fit_transform(X)  # features are already z-score normalized
...
def train_baseline_0(
    ...
    """Train and evaluates a baseline from scratch using LOO cross validation."""
    if cfg.baseline.estimator not in BASELINES:
        raise KeyError(f"Invalid baseline (got '{cfg.baseline.estimator}')")
...
def run(cfg: DictConfig) -> None:
    ...
    preds = train_baseline(cfg, D_cell=D_cell, D_pdxo=D_pdxo, all_drug_ids=all_drug_ids)

    log.info(f"Fitting baseline (0)...")
    preds_0 = train_baseline_0(
        cfg, D_cell=D_cell, D_pdxo=D_pdxo_0, all_drug_ids=all_drug_ids
    )

    results_df = pd.concat([preds, preds_0])
