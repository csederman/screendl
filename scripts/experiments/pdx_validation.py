...
def load_pdx_data(cfg: DictConfig, pdmc_ds: Dataset) -> Dataset:
    """Load the raw PDX screening data and align it to the PDMC feature space."""
...
def prepare_experiment(cfg: DictConfig) -> ExperimentData:
    ...
    # Only evaluate PDXO drugs that are either in PDX or source cell-line screens.
...
def initialize_drug_selector(
    ...
    ctx.drug_selector = SELECTORS[opt_screen.selector](
        exp.dataset.select_cells(D_t_pdxo.cell_ids).select_drugs(exp.D_pdxo.drug_ids),
...
def run_base_predictions(
    ...
    *,
    ...
    ctx.R_pdxo_base = eval_utils.get_predictions_vs_background(
        M=exp.M_base,
    ...
    ctx.R_pdxo_base = _annotate_fold(
        ctx.R_pdxo_base,
        fold_i=fold_i,
        heldout_cell_id=heldout_cell_id,
    )
    write_predictions_chunk(ctx.R_pdxo_base, output_pdxo_path)

    ctx.R_pdx_base = eval_utils.get_predictions_vs_background(
        M=exp.M_base,
    ...
    ctx.R_pdx_base = _annotate_fold(
        ctx.R_pdx_base,
        fold_i=fold_i,
        heldout_cell_id=heldout_cell_id,
    )
    write_predictions_chunk(ctx.R_pdx_base, output_pdx_path)
...
def _fit_function_aux_transfer_model(
    ...
    """Fold-specific response + drug/cell function-aux transfer path."""
    hp_tune = cfg.xfer.hyper
