...
class RunContext:
    ...
    R_screen_no_xfer: pd.DataFrame | None = None

    M_aux: keras.Model | None = None
    M_tune: keras.Model | None = None
    M_screen: keras.Model | None = None
    M_screen_no_xfer: keras.Model | None = None

    W_tune: t.Any = None
    W_screen: t.Any = None
    W_screen_no_xfer: t.Any = None
    ...
    def clear(self) -> None:
        ...
        model_utils.clear_compiled_model(self.M_screen_no_xfer)

        self.R_base = None
        self.R_tune = None
        self.R_screen = None
        self.R_screen_no_xfer = None

        self.M_aux = None
        self.M_tune = None
        self.M_screen = None
        self.M_screen_no_xfer = None

        self.W_tune = None
        self.W_screen = None
        self.W_screen_no_xfer = None
...
def prepare_screenahead_dataset(
    cfg: DictConfig,
    ctx: RunContext,
    dataset: Dataset,
    D_pdxo: Dataset,
    ...
    *,
    ...
    """Select the held-out tumor's ScreenAhead drugs once per fold."""
...
def _fit_screenahead_from_weights(
    cfg: DictConfig,
    *,
    source_model: keras.Model,
    source_weights: t.Any,
    screen_dataset: Dataset,
) -> tuple[keras.Model, t.Any]:
    """Configure and fit a ScreenAhead model from explicit source weights."""
    hp_screen = cfg.screenahead.hyper

    screen_model = model_utils.configure_screenahead_model(
        source_model,
        initial_weights=source_weights,
    ...
    if screen_model is source_model:
        log.warning("ScreenAhead model is the same object as its source model.")

    screen_model = model_utils.fit_screenahead_model(
        model=screen_model,
    ...
    return screen_model, screen_model.get_weights()


def _write_screenahead_predictions(
    cfg: DictConfig,
    ctx: RunContext,
    output_path: Path,
    *,
    model: keras.Model,
    screen_weights: t.Any,
    background_weights: t.Any,
    model_label: str,
    fold_i: int,
    heldout_cell_id: str,
) -> pd.DataFrame:
    """Write one ScreenAhead prediction chunk."""
    if ctx.D_e_pdxo_full is None or ctx.D_t_pdxo_full is None:
        raise RuntimeError("Fold datasets have not been prepared.")
    if ctx.D_s_pdxo is None:
        raise RuntimeError("ScreenAhead dataset has not been prepared.")

    result = eval_utils.get_predictions_vs_background(
        M=model,
        D_t=ctx.D_e_pdxo_full,
        D_b=ctx.D_t_pdxo_full,
        W_t=screen_weights if cfg.experiment.background_correction else None,
        W_b=background_weights if cfg.experiment.background_correction else None,
        model=model_label,
    )
    result["was_screened"] = result["drug_id"].isin(ctx.D_s_pdxo.drug_ids)
    result = _annotate_fold(
        result,
        fold_i=fold_i,
        heldout_cell_id=heldout_cell_id,
    )
    write_predictions_chunk(result, output_path)
    return result
...
def run_screenahead_model(
    ...
    *,
    ...
    """Fine-tune ScreenAhead from the transfer model and write predictions."""
    if ctx.M_tune is None or ctx.W_tune is None:
        raise RuntimeError("Transfer model has not been fit.")
    if ctx.D_s_pdxo is None:
        raise RuntimeError("ScreenAhead dataset has not been prepared.")

    ctx.M_screen, ctx.W_screen = _fit_screenahead_from_weights(
        cfg,
        source_model=ctx.M_tune,
        source_weights=ctx.W_tune,
        screen_dataset=ctx.D_s_pdxo,
    )

    model_utils.save_keras_model(ctx.M_screen, model_dir / "screenahead.keras")

    ctx.R_screen = _write_screenahead_predictions(
        cfg,
        ctx,
        output_path,
        model=ctx.M_screen,
        screen_weights=ctx.W_screen,
        background_weights=ctx.W_tune,
        model_label="screen",
        fold_i=fold_i,
        heldout_cell_id=heldout_cell_id,
    )
...
def run_screenahead_model_without_transfer(
    cfg: DictConfig,
    ctx: RunContext,
    M_base: keras.Model,
    W_base: t.Any,
    ...
    *,
    ...
    """Fine-tune ScreenAhead directly from the pretrained base model."""
    if ctx.D_s_pdxo is None:
        raise RuntimeError("ScreenAhead dataset has not been prepared.")

    ctx.M_screen_no_xfer, ctx.W_screen_no_xfer = _fit_screenahead_from_weights(
        cfg,
        source_model=M_base,
        source_weights=W_base,
        screen_dataset=ctx.D_s_pdxo,
    )

    model_utils.save_keras_model(
        ctx.M_screen_no_xfer,
        model_dir / "screenahead_no_xfer.keras",
    )

    ctx.R_screen_no_xfer = _write_screenahead_predictions(
        cfg,
        ctx,
        output_path,
        model=ctx.M_screen_no_xfer,
        screen_weights=ctx.W_screen_no_xfer,
        background_weights=W_base,
        model_label="screen_no_xfer",
        fold_i=fold_i,
        heldout_cell_id=heldout_cell_id,
    )
...
def run_fold(
    ...
    *,
    ...
    try:
        ...
        run_screenahead_model_without_transfer(
            cfg=cfg,
            ctx=ctx,
            M_base=exp.M_base,
            W_base=exp.W_base,
