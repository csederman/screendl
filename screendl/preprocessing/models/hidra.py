...
@dataclass
class _PathConfig:
    """Path config for HiDRA input features."""
...
@dataclass
class _ParamConfig:
    """Param config for HiDRA input features.."""

    use_gene_symbol_map: bool = True


@dataclass
class HiDRAFeatureConfig:
...
def generate_and_save_hidra_inputs(
    cfg: HiDRAFeatureConfig,
    out_dir: Path,
    exp_df: pd.DataFrame,
    drug_meta: pd.DataFrame,
) -> None:
    """Generates and saves HiDRA input features."""
    ...
    gs_map = HIDRA_GENE_SYMBOL_MAP if params.use_gene_symbol_map else None
    gs_dict = read_hidra_gmt(paths.gmt, gene_symbol_map=gs_map)

    exp_feat, gs_meta, mol_feat = generate_hidra_inputs(
        exp_df=exp_df, exp_gene_sets=gs_dict, drug_info_df=drug_meta
    )

    exp_feat.to_csv(out_dir / "FeatureGeneExpression.csv")
    mol_feat.to_csv(out_dir / "FeatureMorganFingerprints.csv")

    with open(out_dir / "MetaGenesetDict.pkl", "wb") as fh:
        pickle.dump(gs_meta, fh)
