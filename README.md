...
# download data in current working directory and then:
unzip ./screendl-data.zip -d ./data/
```
...
To train and evaluate ScreenDL on all train/val/test folds, run:

```{bash}
python scripts/runners/run.py -m _datastore_="$(pwd)/data"
```
...
```{bash}
python scripts/experiments/pdxo_validation.py -m _datastore_="$(pwd)/data"
```
...
```{bash}
python scripts/experiments/pdx_validation.py -m _datastore_="$(pwd)/data"
```
